...
export const analyzeArtwork = (src: string, catalog: ThreadRef[]) =>
  new Promise<ArtworkAnalysis | null>((resolve) => {
    ...
    image.onload = () => {
      ...
      const base: Omit<ArtworkAnalysis, "coverage" | "palette" | "threads"> = {
        aspect,
        colorCount: 1,
        isVector,
        pixelWidth: width,
      ...
      candidates.forEach((bucket) => {
        ...
        const host = realColors.find((cluster) => {
          ...
          const dGreen = avg(bucket.green) - cluster.green / cluster.count;
          const dBlue = avg(bucket.blue) - cluster.blue / cluster.count;

          return (
            dRed * dRed + dGreen * dGreen + dBlue * dBlue <= MERGE_DISTANCE_SQ
          );
        });
      ...
      const palette = [...realColors]
        ...
        .map((bucket) => ({
          ...
          share: Math.round((bucket.count / covered) * 100) / 100,
        }));
