...
const fabricHeight = (kind: FabricKind, xPos: number, yPos: number) => {
  ...
  if (kind === "rib")
    ...
    return (
      0.5 + 0.5 * Math.sin(xPos * tau * 16 + 0.6 * Math.sin(yPos * tau * 2))
    );
  if (kind === "twill")
  ...
  if (kind === "canvas")
    ...
    return (
      0.5 + 0.25 * Math.sin(xPos * tau * 24) + 0.25 * Math.sin(yPos * tau * 24)
    );

  // Jersey: fine vertical wales with gentle courses across.
  return (
    0.5 + 0.35 * Math.sin(xPos * tau * 32) + 0.15 * Math.sin(yPos * tau * 8)
  );
};
