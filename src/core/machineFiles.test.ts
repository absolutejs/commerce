...
const buildPesBytes = () => {
  ...
  encoder.encode(`LA:${"HALFTONE".padEnd(16, " ")}`).forEach((byte, index) => {
...
describe("parseMachineFile", () => {
  it("routes .dst to the DST header parser (case-insensitive)", () => {
    expect(parseMachineFile(buildDstBytes(), "tee.DST")?.stitches).toBe(123);
  });

  it("routes .exp to the EXP stream parser", () => {
    expect(parseMachineFile(buildExpBytes(), "square.exp")?.stitches).toBe(10);
  });

  it("routes .pes to the PES parser", () => {
    expect(parseMachineFile(buildPesBytes(), "logo.pes")?.stitches).toBe(4);
  });
