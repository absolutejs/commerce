...
const vinylFacts = (inks: InkColor[]): VinylFacts => ({
	...
	minDetail:
		'strokes ≥ 1mm (~3pt) — thinner breaks during weeding; text ≥ 8pt',
	mirror: true,
	pressSpec:
...
export const buildPrintSpec = (
	...
	const sublimation =
		method === 'sublimation'
			? sublimationFacts(darkGarment, fabric)
			: null;
...
export const buildOrderProductionSpec = (
	...
	return {
		...
		items: items.map((item) => ({
			...
			names: item.names,
			placements: item.placements.map((place) =>
				placementSpec(item, place)
			),
...
export const threadSequenceText = (spec: OrderProductionSpec) => {
	const embroidery = spec.items.filter(
		(item) => item.method === 'embroidery'
	);
	const lines = [
...
export const printSheetText = (spec: OrderProductionSpec) => {
	const printItems = spec.items.filter(
		(item) => item.method !== 'embroidery'
	);
	const lines = [
		'PRINT PRODUCTION SHEET',
		...
		...(printItems.length === 0
			...
			: printItems.flatMap((item) =>
					item.placements.flatMap((place) =>
						printPlacementLines(item, place)
					)
				))
	];
...
export const workOrderMarkdown = (
	...
	if (header) {
		...
		if (header.customerEmail)
			lines.push(`- Customer: ${header.customerEmail}`);
	...
	spec.items.forEach((item, itemIndex) => {
		...
		item.placements.forEach((place) => {
			...
			if (place.print) {
				...
				print.warnings.forEach((warning) =>
					lines.push(`- ⚠ ${warning}`)
				);
			}
