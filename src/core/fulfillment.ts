...
export type FulfillmentShippingMethod = {
  /** Stable provider value passed back as FulfillmentOrderRequest.shippingMethod. */
  id: string;
  name: string;
  description?: string;
  carrier?: string;
...
export type FulfillmentShippingMethodRequest = Omit<
  FulfillmentCostQuoteRequest,
  "shippingMethod"
>;

/**
 * Discovers provider-supported shipping choices for the current destination and
 * basket. Discovery is read-only and does not reserve capacity or price.
 */
export interface FulfillmentShippingMethodProvider {
  listShippingMethods(
    order: FulfillmentShippingMethodRequest,
  ): Promise<FulfillmentShippingMethod[]>;
}
