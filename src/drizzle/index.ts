...
export const commercePaymentWebhookConnections = pgTable(
  ...
  (table) => [
    ...
    index("commerce_payment_webhook_connections_reconcile_idx").on(
      table.next_reconciliation_at,
      table.reconciliation_lease_expires_at,
    ),
...
export const commercePaymentWebhookConnectionEvents = pgTable(
  "commerce_payment_webhook_connection_events",
  {
    ...
    metadata: portableJsonb().$type<Record<string, unknown>>().notNull(),
    owner_key: varchar({ length: 160 }).notNull(),
  },
  (table) => [
    index("commerce_payment_webhook_connection_events_owner_idx").on(
      table.owner_key,
      table.created_at,
    ),
    index("commerce_payment_webhook_connection_events_installation_idx").on(
      table.installation_id,
      table.created_at,
    ),
  ],
...
export const commerceStorefrontCases = pgTable(
  "commerce_storefront_cases",
  {
    ...
    closed_at: timestamp({ precision: 3, withTimezone: true }),
    created_at: timestamp({ precision: 3, withTimezone: true })
      .notNull()
      .defaultNow(),
    due_at: timestamp({ precision: 3, withTimezone: true }),
    id: uuid().defaultRandom().primaryKey(),
    idempotency_key: varchar({ length: 200 }).notNull(),
...
export const commerceStorefrontCaseEvents = pgTable(
  "commerce_storefront_case_events",
  {
    ...
    lease_expires_at: timestamp({ precision: 3, withTimezone: true }),
    next_attempt_at: timestamp({ precision: 3, withTimezone: true }),
    notified_at: timestamp({ precision: 3, withTimezone: true }),
    order_id: uuid().notNull(),
    owner_key: varchar({ length: 160 }).notNull(),
    payload: portableJsonb()
      .$type<Record<string, unknown>>()
...
export const commerceDrizzleSchema = {
  ...
  paymentWebhookConnectionEvents: commercePaymentWebhookConnectionEvents,
