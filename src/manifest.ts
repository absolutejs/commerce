  ...
  lifecycle: [
    {
      ...
      kind: "migration",
      title:
        "Add the commerce tables to your Drizzle schema and run `drizzle-kit push`",
  ...
  tools: {
    inventory_levels: tool.runtime({
      ...
      handler: async ({ lowOnly }, db) => {
        const rows = await db
          ...
          .where(
            lowOnly === true
              ? lte(commerceInventory.quantity, commerceInventory.low_threshold)
              : undefined,
          )
          .orderBy(commerceInventory.product_id, commerceInventory.size);
    ...
    list_orders: tool.runtime({
      ...
      handler: async ({ limit, status }, db) => {
        ...
        return rows.length === 0 ? "no orders found" : JSON.stringify(rows);
      },
      input: Type.Object({
        limit: Type.Optional(
